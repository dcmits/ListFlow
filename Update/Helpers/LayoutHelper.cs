    ...
    public static class LayoutHelper
    {
        ...
        private static void GetControlsList(Visual control, int level)
        {
            ...
            for (int i = 0; i <= ChildNumber - 1; i++)
            {
                ...
                //        // Deux lignes.
                //        //<text transform="matrix(1 0 0 1 451.4932 252.3446)"><tspan x="0" y="0" class="st8 st10">#10 </tspan><tspan x="0" y="4.8" class="st8 st10">Test</tspan></text>
                //        // Simple ligne.
                ...
                //    }
                //}
                else if(v is AssignmentUC)
                {
                    AssignmentUC assignmentUC = v as AssignmentUC;

                    //sbListControls.AppendLine(string.Format(@"<text transform=""matrix({0} {1} {2} {3} {4} {5})"" fill=""{6}"" font-family=""{7}"" font-size=""{8}""/>",
                    //     0, -1, 1, 0, Canvas.GetTop(assignmentUC),  Canvas.GetLeft(assignmentUC), assignmentUC.FontFamily, assignmentUC.FontSize));

                    //<text class="cls-7" transform="translate(344.87 258.82) rotate(-90)">#Montext</text>

                    if (assignmentUC.IsEnabled)
                    {
                        // Espace occupé par le texte.
                        Rect textArea = assignmentUC.GetRect(assignmentUC.Parent as Canvas);

                        double offsetY = 0;

                        if (assignmentUC.ContentAlignment == HorizontalAlignment.Right && assignmentUC.Rotate == 0)
                        {
                            offsetY = textArea.Height / 2;
                        }
                        //else if (assignmentUC.HorizontalAlignment == HorizontalAlignment.Right && assignmentUC.Rotate == 0)
                        //{

                        //}

                        if (assignmentUC.Available)
                        {
                            sbListControls.AppendLine(string.Format(@"<text transform=""translate({0} {1}) rotate({2})"" fill=""{3}"" font-family=""{4}"" font-size=""{5}"">{6}</text>",
                                     textArea.X, textArea.Y + offsetY, assignmentUC.Rotate, "#000000", assignmentUC.FontFamily, assignmentUC.FontSize, assignmentUC.Text));
                        }
                        else
                        {
                            // Console.WriteLine(assignmentUC);


                            sbListControls.AppendLine(string.Format(@"<text transform=""translate({0} {1}) rotate({2})"" fill=""{3}"" font-family=""{4}"" font-size=""{5}"">{6}</text>",
                                     textArea.X, textArea.Y + offsetY, assignmentUC.Rotate,
                                     "#FF4D00", assignmentUC.FontFamily, assignmentUC.FontSize, assignmentUC.Text));

                            //sbListControls.AppendLine(string.Format(@"<text transform=""translate({0} {1}) rotate({2})"" fill=""{3}"" font-family=""{4}"" font-size=""{5}"">{6}</text>",
                            //         Canvas.GetLeft(assignmentUC) + assignmentUC.Width, Canvas.GetTop(assignmentUC) + (assignmentUC.Height / 2), assignmentUC.Rotate,
                            //         "#FF4D00", assignmentUC.FontFamily, assignmentUC.FontSize, assignmentUC.Text));
                        }

                        // Console.WriteLine(assignmentUC.ToString());
                    }
